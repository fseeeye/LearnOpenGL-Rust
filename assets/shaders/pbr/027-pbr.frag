...
const float MAX_PREFILTERED_MAP_LOD = 4.0;
...
// G term use The Smith shadowing-masking algorithm
...
void main()
{
    ...
    // Diffuse part of ambient lighting
    vec3 Ks = FresnelTerm(max(dot(N, V), 0.0), F0);
    vec3 Kd = (1.0 - Ks) * (1.0 - metallic);
    vec3 ambient_diffuse_irradiance = texture(irradiance_map, N).rgb;
    vec3 ambient_diffuse = Kd * ambient_diffuse_irradiance * albedo;
    // Specular part of ambient lighting
    vec3 prefiltered_lighting = textureLod(prefiltered_envmap, R, roughness * MAX_PREFILTERED_MAP_LOD).rgb;
    vec2 prefiltered_brdf = texture(brdf_lut, vec2(max(dot(N, V), 0.0), roughness)).rg;
    vec3 ambient_specular = prefiltered_lighting * (F0 * prefiltered_brdf.x + prefiltered_brdf.y);
